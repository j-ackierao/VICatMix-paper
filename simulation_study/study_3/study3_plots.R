...
simsplot <- lapply(sim1novarsel, "[[", 1)
simsplot <- bind_rows(simsplot, .id = "data_number")
simsplot <- simsplot[simsplot$model != "FlexMixBIC",]
...
ggplot(simsplot, aes(x = model, y = Clusters)) +geom_hline(yintercept=20, linetype='dashed', col = 'red') + stat_boxplot(geom = "errorbar", width = 0.5) + geom_boxplot(inherit.aes = TRUE, fatten = 1, fill = "lightsalmon") + theme_bw()

#Variable selection example
simsvsplot <- lapply(sim1varsel, "[[", 1)
