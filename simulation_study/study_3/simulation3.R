#Simulation 3 (no variable selection - n = 2000)
...
sim3novarsel <- foreach(i = 1:10) %dorng% {
  ...
  start.time <- Sys.time()
  premiumModel <- profRegr(yModel=inputs$yModel, xModel=inputs$xModel, data=inputs$inputData, nSweeps=2500, nClusInit=30, nBurn=1000, output=paste0("sim_output/sim2000_", i), covNames = inputs$covNames, reportBurnIn = TRUE, excludeY = TRUE)
  dissimObj <-calcDissimilarityMatrix(premiumModel)
  clusObj   <-calcOptimalClustering(dissimObj, maxNClusters=30)
  end.time <- Sys.time()
  ...
  #BayesBinMix
  start.time <- Sys.time()
  bayesbin <- coupledMetropolis(30, 6, seq(1, 0.6, length = 6), data, outPrefix = paste0("BayesBinSim2000_", i), ClusterPrior = "poisson", m = 2500, burn = 500)
  end.time <- Sys.time()
  ...
  #flexmix
  start.time <- Sys.time()
  flex <- initFlexmix(data ~ 1, k = 1:30, model = FLXMCmvbinary(), control = list(minprior = 0), nrep = 10)
...
save(sim3novarsel, file="/home/jr951/VariationalMixtures/sim3novarsel.RData")