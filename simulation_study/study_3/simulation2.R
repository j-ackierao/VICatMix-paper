#Simulation 2 (no variable selection - uneven clusters)
...
sim2novarsel <- foreach(i = 1:10) %dorng% {
  ...
  start.time <- Sys.time()
  premiumModel <- profRegr(yModel=inputs$yModel, xModel=inputs$xModel, data=inputs$inputData, nSweeps=2500, nClusInit=20, nBurn=1000, output=paste0("sim_output/unevensim_", i), covNames = inputs$covNames, reportBurnIn = TRUE, excludeY = TRUE)
  ...
  #BayesBinMix
  start.time <- Sys.time()
  bayesbin <- coupledMetropolis(20, 6, seq(1, 0.6, length = 6), data, outPrefix = paste0("BayesBinSimUneven_", i), ClusterPrior = "poisson", m = 2500, burn = 500)
  end.time <- Sys.time()
