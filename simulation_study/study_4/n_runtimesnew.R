...
n_runtimesnew <- foreach(i = 1:20) %dorng% {
  runtimes_n <- list()
  for (j in 1:8){
    generation <- generateSampleDataBin(n_list[j], 10, c(0.1, 0.2, 0.1, 0.2, 0.05, 0.05, 0.05, 0.05, 0.1, 0.1), 100, 0)
    generation2 <- generateSampleDataBin(n_list[j], 10, c(0.1, 0.2, 0.1, 0.2, 0.05, 0.05, 0.05, 0.05, 0.1, 0.1), 80, 20)
    data <- generation[[1]]
    ...
    start.time <- Sys.time()
    clustering <- runVICatMix(data, 20, 0.01, tol = 0.00005)
    end.time <- Sys.time()
    ...
    modelresult <- data.frame(Model = c("NVS", "VS"), n_val = n_list[j], ELBO = NA, Clusters = NA, ARI = NA, Time = NA)
...
save(n_runtimesnew, file="/home/jr951/VariationalMixtures/n_runtimesnew.RData")
