...
cvals_sim <- foreach(i = 1:10) %dorng% {
  ...
  generation <- generateSampleDataBin(1000, 10, rep(0.1, 10), 75, 25)
  data <- generation[[1]]
  truelabels <- generation[[2]]
  results_list <- list()
  for(j in 1:10){
    #10 different initialisations on each dataset
    run <- lapply(X = cvals, FUN = runVICatMix, data = data, K = 20, alpha = 0.1, tol = 0.00005)
...
save(cvals_sim, file="cvals_sim.RData")